...
def run(X_ini, Y_ini, X_test,Y_test,pf_test,num_H,num_val,N_mont, training_epochs=300, batch_size=5000, LR=0.001, LRdecay=0, K=1, layernum=[2]):
    global mlp
    ...
    Ypred = np.zeros((1000,K,N_mont))  
    X_test = np.transpose(X_test) 
    pf_test = np.transpose(pf_test) 
    Y_test = np.transpose(Y_test) 
    Ora_meansop = SOP(pf_test,Y_test,0.1*K)
    for i_mont in range(N_mont):
        ...
        mlp = MLP(layernum, [1, 10, 1],
                  0.1, 5*K, K, transfer_function=tf.nn.softplus,
        ...
        NN_meansop = SOP(pf_test,y_pred,0.1*K)        
